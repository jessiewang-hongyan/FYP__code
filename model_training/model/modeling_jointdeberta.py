...
class JointDeberta(DebertaPreTrainedModel):
    def __init__(self, config, args, intent_label_lst, slot_label_lst):
        super(JointDeberta, self).__init__(config)
        self.args = args
        self.num_intent_labels = len(intent_label_lst)
        self.num_slot_labels = len(slot_label_lst)
        self.roberta = DebertaModel(config=config)  # Load pretrained bert
    ...
    def forward(self, input_ids, attention_mask, intent_label_ids, slot_labels_ids):
        outputs = self.roberta(input_ids, attention_mask)  # last-layer hidden-state, (), (attentions)
